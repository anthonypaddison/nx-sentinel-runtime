...
- Removed duplicate WS config GET retry in `_getStoredConfig` and added a one-time toast (`Backend config unavailable`) when WS config endpoints are unreachable.
- Changed config/prefs/data keys to user-scoped format and added one-time migration from legacy device-scoped keys (current device) for localStorage + IndexedDB.
- Updated reset logic to clear both user-scoped keys and legacy device-scoped keys for the same user, including IndexedDB entries matched by prefix.
- Added `tests/persistence-user-scope.test.mjs` for user-scoped key generation, migration copy behavior, and reset cleanup coverage.
...
- Prevent cross-device divergence for the same Home Assistant user while keeping existing data through backward-compatible migration.
...
10. With backend WS config endpoints disabled, confirm only one fallback toast is shown and card continues using local/IDB config without repeated load-path retries.
11. Save config on one device and confirm the same user sees matching config/prefs/data on another device after reload.
12. Run `node --test tests/*.mjs` and verify `persistence-user-scope` tests pass.
